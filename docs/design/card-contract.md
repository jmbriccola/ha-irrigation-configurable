...
| `hub_water_total` | sensor | liters (float), `device_class: water`, `state_class: total_increasing` | — every zone's cumulative litres, summed. Unattributed water is excluded: it is not consumption, and folding a leak in would make a leak look like irrigation. Today/this-month are not attributes here — a card composes them from the per-zone `today_l`/`month_l` it already reads |
...
| maestro_role        | platform | state | extra attributes |
|---------------------|----------|-------|------------------|
| `zone_state`        | sensor   | `idle` \| `queued` \| `watering` \| `soaking` \| `paused` \| `suspended` \| `disabled` | `zone_name`, `order`, `adjustment_pct` (float, 10–300), `degraded` (list of keys, see below), `run_started_at` (ISO, while watering), `run_duration_min` (frozen total), `run_planned_runs` (soak split list), `active_cycle_id`, `suspended_until` (ISO or null), `cycles` (list, see below), `capabilities` (object), `next_run` (object), `valve_entity` (the entity id of the zone's valve — "the zone is watering" and "the valve is open" are different facts, and a card should render both and say when they disagree: a zone reading `idle` whose valve reads `open` is the shape of a failed close) |
